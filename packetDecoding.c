...
int decodePacket(uint64_t packets[], size_t packetCount, uint8_t deviceId) {
    ...
    int pos,count;
    int header, footer, id;
   for (size_t i = 0; i < packetCount; i++)
   {
    header = ((packets[i] >> HEADER_SHIFT) & HEADER_MASK );
    id = ((packets[i] >> DEVICE_ID_SHIFT) & DEVICE_ID_MASK );
    footer = ((packets[i] >> FOOTER_SHIFT) & FOOTER_MASK );
    //printf("%d", header);
    if ( header == HEADER && id ==DEVICE_ID && footer == FOOTER )
    {
        pos = ((packets[i] >> ORDER_SHIFT) & ORDER_MASK);
        decodedData[pos] = ((packets[i] >> DATA_SHIFT) & DATA_MASK);
    }
    else
    {
        return -1;
